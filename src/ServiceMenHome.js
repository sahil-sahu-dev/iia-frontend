...
const ServiceMenHomePageComponent = () => {
    ...
    const bookingsClicked = ()=>{
        navigate('/serviceMenBookings', { state: { id } });
    }
    ...
    useEffect( () => {
        ...
        const fetchData = async () => {
            const response2 = await fetch('http://127.0.0.1:5000/fetch_pending_booking/' + id);
  ...
  return (
    <div>
      ...
      <div className="p-5 text-center bg-body-tertiary rounded-3">
        <h1 className="text-body-emphasis">Welcome Servicemen!</h1>
      </div>
        ...
        <tbody>
        {
            entries.map((entry, index)=>{
              ...
              const handleAccept = async ()=>{
                const response2 = await fetch('http://127.0.0.1:5000//accept_booking/' + bookingId);
                const result2 = await response2.json();
                alert(JSON.stringify(result2));
              ...
              const handleReject = async ()=>{
                const response2 = await fetch('http://127.0.0.1:5000//reject_booking/' + bookingId);
                const result2 = await response2.json();
                alert(JSON.stringify(result2));
                entries.splice(index,1);
                setEntries(entries);
...
export default ServiceMenHomePageComponent;
